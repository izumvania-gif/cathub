...
2. Во вкладке «Переменные» (лучше как секреты):
   ...
   - `APP_URL` — адрес приложения (`https://…amvera.io`): бот вставляет его в сообщения и ставит кнопку меню «CatHub», которая открывает приложение внутри Telegram (Mini App, вход без пароля для тех, кто подключил Telegram).
